...
More importantly, **Claude Code and Cowork make it economically viable to build custom financial software** that goes 10x deeper on what RI actually needs. Rather than accepting QBO's constraints and paying for features RI will never use, RI can build personal software optimized for affordable housing operations, fund accounting for restricted grants, and integration with existing internal tools (renewal-timesheets, expense-reports-homegrown, app-portal).
...
The following invariants are enforced at the database or application level. They are not configurable, not overridable, and not subject to user preference. Engineers should treat violations of any invariant as a system bug, not a user error.

1. **Double-entry balance**: For every transaction, `Sum(debits) = Sum(credits)`. The system rejects any transaction that violates this at write time. *(GL-P0-001)*

2. **Valid account reference**: Every transaction line must reference an `account_id` that exists in the chart of accounts. *(GL-P0-002)*

3. **Valid fund reference**: Every transaction must reference a `fund_id` that exists in the funds table. *(GL-P0-003)*

4. **Active account constraint**: Inactive accounts cannot receive new transaction lines. Existing history is retained, but new debits/credits are rejected. *(GL-P0-002)*

5. **Fund restriction immutability**: A fund's restriction type (Restricted or Unrestricted) is set at creation and cannot be changed afterward. If a fund's restriction status changes in practice (rare), create a new fund and transfer balances. *(GL-P0-003)*

6. **Posted transaction immutability**: No posted transaction may be edited or deleted. Corrections are made exclusively via reversal entries. A reversal entry must: (a) create a new transaction with equal and opposite debits/credits, (b) store a reference to the original transaction being reversed (`reversal_of_transaction_id`), and (c) be flagged as a reversal in the ledger for audit trail purposes.

7. **Restricted fund auto-release**: When an expense is posted to a restricted fund, the system must atomically generate a corresponding net asset release entry (DR: Net Assets With Donor Restrictions, CR: Net Assets Without Donor Restrictions) for the same amount. No restricted-fund expense may exist without its paired release. *(GL-P0-005)*

8. **System-generated entry flag**: All entries generated by the system (depreciation, loan interest accrual, net asset releases, FY25 conversion adjustments) must be flagged `is_system_generated = true`. System-generated entries are non-editable by users. Manual corrections to system-generated entries must be made via separate adjusting journal entries, not by modifying the original. *(GL-P0-008, GL-P0-011, GL-P0-005, GL-P0-018)*

9. **Ramp posting gate**: Ramp credit card transactions in "pending" status (uncategorized) are excluded from financial statements and cannot be posted to the GL. Categorization is a prerequisite for posting. *(GL-P0-020)*

10. **Fund-level trial balance integrity**: For any individual fund, `Sum(debits) = Sum(credits)`. This follows from invariants 1 and 3 but is stated explicitly because fund-level reporting depends on it.

11. **Transaction source provenance**: Every transaction must record its origin via `source_type` (e.g., `MANUAL`, `TIMESHEET`, `EXPENSE_REPORT`, `RAMP`, `SYSTEM`) and an optional `source_reference_id` (the external system's identifier for the originating record). These fields support integration traceability (Chunk 8) and audit trail requirements. They are set at creation and are immutable.

12. **Immutable audit log**: Every GL write operation (transaction created, reversal posted, account created/deactivated, fund created/deactivated) must be recorded in an append-only audit log with: timestamp, acting user, action type, and affected entity ID. The audit log cannot be edited or deleted. This is separate from the ledger itself and exists for operational accountability.
...
**Requirement**: The ledger supports full fund accounting with dynamic fund creation. Each transaction is coded to exactly one fund at the transaction header level (not per-line). All lines within a transaction inherit the header's fund. Funds are designated "Restricted" or "Unrestricted" at creation. Fund-level balances roll up to net asset accounts ("Net Assets With Donor Restrictions" and "Net Assets Without Donor Restrictions").
...
- [x] Funds can be deactivated (preventing new transactions) only when their balance is zero. Deactivated funds retain full transaction history and remain visible in historical reports. Funds with non-zero balances cannot be deactivated — balances must first be transferred to another fund via journal entry.
...
- [x] System stores depreciation schedules: asset name, original cost, salvage value (defaults to $0 if not specified), useful life (years), depreciation method (straight-line, etc.), date placed in service, monthly depreciation amount
- [x] System generates monthly depreciation entries automatically based on configured schedules
- [x] Depreciation entries post to **General Fund (Unrestricted)** — depreciation is an entity-level operating expense, not a restricted-fund expenditure, regardless of which fund was used to acquire the asset
...
- And the entry posts to **General Fund (Unrestricted)** — loan interest is an operating expense of the organization, not a restricted-fund expenditure, regardless of the fund associated with the loan's capital purpose
...
| Transaction Type | Fund Required? | GL Account Required? | Default Fund | Posting Behavior |
|---|---|---|---|---|
| Rental Income | **Yes** (enforced) | Yes | None — must select | Posts immediately on entry |
| Ramp Credit Card | **Yes** (enforced before posting) | **Yes** (enforced — categorization required) | None — must select | Holds in "pending" until categorized, then posts |
| Timesheet / Payroll | Yes (defaulted) | Salaries & Wages + liability accounts (system-determined) | General Fund (Unrestricted) | Batched monthly from approved timesheets |
| Expense Report | Yes (warn if missing) | Yes | None — user selects | Posts on approval in source app |
| Manual Journal Entry | Yes (warn if missing) | Yes | None — user selects | Posts immediately on entry |
| System-Generated (depreciation, interest, releases) | Yes (system-determined) | Yes (system-determined) | Per posting rule (see GL-P0-008, GL-P0-011, GL-P0-005) | Automatic — no user action |
...
- **Resolved**: See System Invariant #6. Posted transactions are immutable. Corrections are made via reversal entries that create equal-and-opposite debits/credits, store a `reversal_of_transaction_id` reference, and are flagged as reversals. No transactions are ever deleted or edited after posting.
...
- **Likelihood**: Medium — Integration is always risky. Employee payroll data sync from app-portal adds complexity.
...
## 11. Appendix: Seed Chart of Accounts

The following accounts are created at system bootstrap. "System-locked" accounts are required for automated entries or core GL mechanics and cannot be deactivated or renamed. "User-managed" accounts can be deactivated (but not deleted if transaction history exists). Users may create additional accounts as needed.

### Assets

| Account Name | Sub-Type | Normal Balance | System-Locked? | Source Requirement |
|---|---|---|---|---|
| Checking | Cash | Debit | No | Core |
| Savings | Cash | Debit | No | Core |
| Accounts Receivable | Current Asset | Debit | Yes | GL-P0-013 |
| Grants Receivable | Current Asset | Debit | Yes | GL-P0-014 |
| Prepaid Expenses | Current Asset | Debit | Yes | GL-P0-015 |
| Construction in Progress | Fixed Asset | Debit | Yes | GL-P0-009 |
| Building | Fixed Asset | Debit | Yes | GL-P0-008 |
| Equipment | Fixed Asset | Debit | No | GL-P0-008 |
| Accumulated Depreciation - Building | Contra-Asset | Credit | Yes | GL-P0-008 |
| Accumulated Depreciation - Equipment | Contra-Asset | Credit | No | GL-P0-008 |

### Liabilities

| Account Name | Sub-Type | Normal Balance | System-Locked? | Source Requirement |
|---|---|---|---|---|
| Accounts Payable | Current Liability | Credit | Yes | GL-P0-004 |
| Credit Card Payable | Current Liability | Credit | Yes | GL-P0-020 |
| Accrued Expenses Payable | Current Liability | Credit | Yes | GL-P0-015 |
| Deferred Revenue | Current Liability | Credit | Yes | GL-P0-015 |
| Accrued Payroll Payable | Current Liability | Credit | Yes | GL-P0-006 |
| Federal Income Tax Payable | Payroll Liability | Credit | Yes | GL-P0-006 |
| State Income Tax Payable | Payroll Liability | Credit | Yes | GL-P0-006 |
| Social Security Payable | Payroll Liability | Credit | Yes | GL-P0-006 |
| Medicare Payable | Payroll Liability | Credit | Yes | GL-P0-006 |
| Workers Comp Payable | Payroll Liability | Credit | No | GL-P0-006 |
| 401(k) Withholding Payable | Payroll Liability | Credit | No | GL-P0-006 |
| AHP Loan Payable | Long-Term Liability | Credit | Yes | GL-P0-010 |
| Accrued Interest Payable | Current Liability | Credit | Yes | GL-P0-011 |

### Net Assets (Equity)

| Account Name | Sub-Type | Normal Balance | System-Locked? | Source Requirement |
|---|---|---|---|---|
| Net Assets Without Donor Restrictions | Unrestricted | Credit | Yes | GL-P0-003, GL-P0-005 |
| Net Assets With Donor Restrictions | Restricted | Credit | Yes | GL-P0-003, GL-P0-005 |

### Revenue

| Account Name | Sub-Type | Normal Balance | System-Locked? | Source Requirement |
|---|---|---|---|---|
| Rental Income | Operating Revenue | Credit | Yes | GL-P0-004 |
| Rental Income - Proration Adjustments | Adjustment (Contra) | Debit | No | GL-P0-016 |
| Rental Income - Hardship Adjustments | Adjustment (Contra) | Debit | No | GL-P0-016 |
| Rental Income - Vacate Adjustments | Adjustment (Contra) | Debit | No | GL-P0-016 |
| Vacancy Loss | Contra-Revenue | Debit | No | GL-P0-007 |
| Grant Revenue | Restricted Revenue | Credit | Yes | GL-P0-014 |
| Donation Income | Contribution Revenue | Credit | Yes | GL-P0-012 |

### Expenses

| Account Name | Sub-Type | Normal Balance | System-Locked? | Source Requirement |
|---|---|---|---|---|
| Salaries & Wages | Payroll | Debit | Yes | GL-P0-006 |
| Interest Expense | Financial | Debit | Yes | GL-P0-011 |
| Depreciation Expense | Non-Cash | Debit | Yes | GL-P0-008 |
| Property Taxes | Property Operations | Debit | No | GL-P0-007 |
| Property Insurance | Property Operations | Debit | No | GL-P0-007 |
| Management Fees | Property Operations | Debit | No | GL-P0-007 |
| Commissions | Property Operations | Debit | No | GL-P0-007 |
| Landscaping & Grounds | Property Operations | Debit | No | GL-P0-007 |
| Repairs & Maintenance | Property Operations | Debit | No | GL-P0-007 |
| Utilities - Electric | Property Operations | Debit | No | GL-P0-007 |
| Utilities - Gas | Property Operations | Debit | No | GL-P0-007 |
| Utilities - Water/Sewer | Property Operations | Debit | No | GL-P0-007 |
| Utilities - Internet | Property Operations | Debit | No | GL-P0-007 |
| Utilities - Security & Fire Monitoring | Property Operations | Debit | No | GL-P0-007 |
| Utilities - Trash | Property Operations | Debit | No | GL-P0-007 |
| Other Operating Costs | Property Operations | Debit | No | GL-P0-007 |

### Seed Funds

| Fund Name | Restriction Type | System-Locked? | Source |
|---|---|---|---|
| General Fund | Unrestricted | Yes (default fund, cannot be deactivated) | GL-P0-003 |
| AHP Fund | Restricted | No | GL-P0-003 |
