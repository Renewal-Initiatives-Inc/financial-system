...
async function getDeferredRevenueEntries(limit = 50) {
  return db
    .select({
      id: transactions.id,
...
export default async function DeferredRevenuePage() {
  const entries = await getDeferredRevenueEntries()
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Funds received but not yet earned.
        </p>
      </div>
      <RecentEntriesTable
        entries={entries}
        emptyMessage="No deferred revenue entries recorded yet."
      />
    </div>
  )
}
