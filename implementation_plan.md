...
13. Build vendor 1099 tracking foundation: query to sum payments per vendor per calendar year. Show running total on vendor detail page. Threshold is configurable per calendar year — $600 for TY2025 and prior, $2,000 for TY2026+ per One Big Beautiful Bill Act (OBBBA § 103, amending IRC § 6041), inflation-indexed from TY2027. Flag when threshold crossed (DM-P0-015)
...
0. Define `annual_rate_config` table (cross-cutting prerequisite for payroll, 1099, and any feature needing year-specific rates): id, fiscal_year (integer, NOT NULL), config_key (varchar(100), NOT NULL), value (numeric(15,6), NOT NULL), effective_date (date, nullable — for mid-year rate changes), notes (text, nullable), updated_by (varchar(255)), updated_at (timestamp). UNIQUE(fiscal_year, config_key, effective_date). Seed with known values:
    - `fica_ss_rate`: 0.062000 (2025, 2026)
    - `fica_medicare_rate`: 0.014500 (2025, 2026)
    - `fica_ss_wage_base`: 176100 (2025), 184500 (2026) — per SSA announcements
    - `vendor_1099_threshold`: 600 (2025), 2000 (2026) — per OBBBA
    - `ma_state_tax_rate`: 0.05 (2025, 2026) — per MA DOR Circular M
    - Build `getAnnualRate(key: string, year: number, asOfDate?: string): number` utility that returns the rate for a given year, with optional mid-year date for rates that change intra-year. Always falls back to most recent prior year if current year not yet configured. Add compliance calendar item "Review and update annual tax rates" (annually, October — when SSA/IRS typically announce next-year rates). Past-year rates are immutable once a payroll run is posted for that year
...
7. Build FICA calculator: 6.2% Social Security (up to annual wage base — check cumulative YTD wages), 1.45% Medicare, both employee and employer share (TXN-P0-035). All rates and the SS wage base MUST be read from the `annual_rate_config` table (see task 0 below), NOT hardcoded. SS wage base for 2026 is $184,500 (SSA announcement Oct 2025). Historical payroll recalculations and reports must use the rate in effect for that calendar year
...
8. Build component depreciation support (DM-P0-020): Lodging building parent with child components. Each component depreciates independently using **GAAP estimated useful lives** (NOT MACRS tax recovery periods — nonprofits use management's best estimate per ASC 360-10-35 / ASC 958-360-35; MACRS only applies to taxable income calculations which 501(c)(3)s generally don't have). Default GAAP lives: structure 40yr, roof 20-25yr, HVAC 20yr, electrical 20-25yr, plumbing 20-25yr, windows 20-25yr, flooring 5-15yr. Editable at CIP conversion. See 2 CFR 200.436 for federal grant compliance (requires GAAP lives). Parent asset detail page shows component breakdown
...
10. Build CIP-to-fixed-asset conversion wizard (`/assets/cip/convert`) per DM-P0-030:
    ...
    - Step 3: For Lodging: allocate CIP balance across components (structure, roof, HVAC, electrical, plumbing, windows, flooring). Set useful life per component using GAAP defaults (structure 40yr, roof 20-25yr, HVAC 20yr, etc.). For Barn/Garage: single-item allocation with 40yr useful life (GAAP building shell estimate)
...
3. Build compliance calendar system (`src/lib/compliance/`):
    - Seed all compliance deadlines per SYS-P0-009: 990 filing (May 15), Form PC (May 15), quarterly 941/M-941, W-2 deadline (Jan 31), 1099-NEC (Jan 31), budget cycle milestones (Sept-Dec), insurance renewals, per-tenant security deposit anniversaries, annual board meeting items, **annual tax rate review (October — SSA announces SS wage base mid-Oct, IRS releases Pub 15-T for next year; update `annual_rate_config` table with next-year rates before January payroll)**, public support trajectory review (~FY2028 — as rental income enters Total Support denominator on Schedule A Line 10a but not Public Support numerator Line 1, RI's public support percentage will decline post-construction; proactive review ensures ratio stays above 33⅓% threshold per IRC § 509(a))
...
7. Build 1099-NEC preparation (TXN-P0-023): auto-track vendor payments by calendar year, flag threshold from `annual_rate_config` table (`vendor_1099_threshold` key — $600 for TY2025, $2,000 for TY2026+ per OBBBA), W-9 collection tracking, data export (CSV), PDF 1099-NEC form generation via pdf-lib. Add TaxBandits API integration (`src/lib/integrations/taxbandits.ts`) as optional IRS e-filing path: W-9 collection via email (`request_w9_by_email`), 1099-NEC validation (`validate_1099_nec`), and IRS e-file submission (`create_1099_nec`). TaxBandits also exposes a remote MCP server (https://developer.taxbandits.com/docs/mcp/) for conversational tax filing if the copilot (Phase 18) needs to invoke it. Resolves open question Q19 from company_facts.md
8. Build filing progression awareness (SYS-P0-010): auto-compute which 990 form type RI must file based on real-time GL data — no manual configuration needed. Thresholds use IRS "normally" sliding scale (per Form 990 instructions and Rev. Proc. 2011-15):
    - **Year 1:** gross receipts for that year only
    - **Years 1-3:** average gross receipts over years of existence
    - **Year 4+:** average gross receipts over prior 3 years (rolling)
    Filing types: 990-N (normally ≤$50K), 990-EZ (normally <$200K AND total assets <$500K at end of year), Full 990 (either threshold exceeded). For total assets test, use end-of-year balance (no averaging). **Key finding from MCP research (Feb 2025):** CIP counts as total assets on Form 990 Part X Line 10 (Schedule D Part VI Line 1e "Other"). RI will trigger Full 990 during construction — the moment CIP soft costs (architecture, engineering, legal, development consulting) push total assets past $500K — not at property PIS date. Comparable org: Falcon Housing Corp (EIN 04-3538884, E. Longmeadow) files Full 990 with similar profile ($578K revenue, $3.3M total assets driven by property). Sub-tasks:
...
5. Build context package for fixed asset form: form state, asset register data, IRS publication search tool, useful life lookup tool. Tools: `regulationLookup`. Knowledge: D-127 depreciation policy, D-080 component depreciation rules, ASC 360-10-35 (GAAP useful life estimation for nonprofits — NOT MACRS tax lives), Pub 946 excerpts for reference only (SYS-P0-004 — replaces standalone depreciation assistant D-128). Note: nonprofit depreciation uses management's best estimate of actual useful life per GAAP, not MACRS recovery periods which apply only to taxable income calculations
...
3. Build security deposit interest automation (TXN-P0-041): annual calculation per tenant on tenancy anniversary. Interest = deposit * rate (lesser of actual bank rate or 5%). GL: DR Interest Expense, CR Cash. Cron job triggers on anniversary date. Compliance calendar integration for 30-day advance warning. **Important:** per MA G.L. c. 186 § 15B, interest accrues from **date of deposit receipt** (the `deposit_date` field on tenant record), NOT from lease start or move-in date. If deposit is collected before move-in, interest accrues from collection date. The anniversary for interest payment purposes is calculated from `deposit_date`, and interest calculations must use `deposit_date` as the accrual start
