...
### D-132: People Service API Built in app-portal — Coexists with D-124 DB Access
- **Date:** 2026-02-13
- **Decision:** A People Service API has been built in app-portal (`/api/v1/people/`) providing RESTful CRUD for employee data. The API uses Zitadel JWT auth (not API keys) and covers: people list/detail/create/update, payroll data with encrypted tax IDs (AES-256-GCM), and field-level audit trails. New database tables: `people` (employment profile, personal info, emergency contacts, skills, hourly rate), `employee_payrolls` (tax IDs, withholding elections, pay frequency), `people_audit_logs` (change tracking with automatic PII masking). This API coexists with D-124's direct-DB-read pattern — the REST API serves use cases where DB access is impractical (admin UI, future external consumers, the app-portal's own admin pages), while D-124's restricted Postgres roles remain the integration path for financial-system and renewal-timesheets. Both access the same underlying tables. The REST API does NOT replace D-124 for internal app integration.
- **Rationale:** D-124 is the right pattern for internal app-to-app data reads (simple, no API layer to maintain, consistent with D-118). But the People Service needs an API surface anyway: the app-portal admin UI needs endpoints for managing people/payroll data, and the schema/encryption/audit-trail infrastructure is needed regardless of access pattern. Building the API now avoids re-doing this work when the admin UI is built. Consuming apps choose their access path: direct DB for high-volume reads (payroll processing, timesheet rate lookups), REST API for occasional operations (if any).
- **New People API fields (extending D-119, D-120):** `compensation_type` (PER_TASK | SALARIED), `annual_salary`, `expected_annual_hours`, `exempt_status` (EXEMPT | NON_EXEMPT), and `calculated_hourly_rate` are NOT YET in the schema — they were identified in D-119/D-120 but need to be added to the `people` table before the timesheets integration is built.
- **Affects:** D-017 (now implemented — employee master data exists in app-portal's DB). D-124 (unchanged — DB access pattern still applies for financial-system and timesheets). OQ-001 from dependencies.md (resolved — PII security architecture is AES-256-GCM encryption at the application layer, stored in `employee_payrolls` table). Chunk 8 spec (must document which tables financial-system and timesheets get SELECT access to). employee-payroll-data-spec.md (archived — superseded by this implementation and D-124).
- **Implementation status:** Schema, encryption, auth middleware, query functions, and API routes are all built in `app-portal/src/`. Needs: `pnpm db:generate` to create migration, `PEOPLE_ENCRYPTION_KEY` env var, and the D-119/D-120 compensation fields added to schema.
